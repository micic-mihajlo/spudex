...
Look for `*-mode/SKILL.md` matching the user's handle, under the project's `~/.codex/skills/` or `~/.codex/skills/`. If one exists, confirm intent with `AskQuestion` (unless they already said "update my skill" or similar):
...
The **spudex** skill shows the shape. Read it for granularity. Do not copy its content; the user's rules are not the same as Spudex's.

### 4. Draft the skill

Use Codex's built-in `skill-creator` skill to author the skill. Placement:

- Path: `~/.codex/skills/<handle>-mode/SKILL.md` in the project (or `~/.codex/skills/<handle>-mode/` if the user prefers a personal skill).
...
- The **spudex** skill: example of the output shape.
