---
name: spudex
description: "Codex operating mode for senior engineering work: concise but complete communication, explicit plans for multi-step tasks, principled design choices, careful delegation, small simple code changes, direct verification, and candid judgment. Use for spudex, /spudex, rigorous codebase work, autonomous implementation, pstack-inspired workflows, or requests for disciplined Codex execution."
---

# Spudex

Spudex is the Codex-native entrypoint for this skill pack. It adapts pstack's engineering discipline to Codex tools, plans, approvals, automations, browser plugins, git directives, and optional multi-agent tools.

## Non-negotiables

**Start every multi-step task by opening a Codex `update_plan` whose first item is to read the Principles section below in full.** The principles ground every trigger below. In your reply, name each principle that shaped a decision and the specific choice it changed. A principle cited with no concrete decision behind it is the tell that you skipped its leaf skill; the citation has to trace to a real choice the leaf's rule drove.
...
- Any prose surface → the **unslop** skill. Your reply to the user is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows the **skill-creator** skill.
- Before commit → use the `deslop` skill when available. If unavailable, do an explicit self-review of the exact outgoing diff before staging or pushing.
- Shipping UI / IDE / CLI → use the matching control skill for your surface. Prefer `control-cli` for CLIs and TUIs. Prefer `control-ui`, Browser, or Chrome for browser, Electron, and web app UIs. For bug fixes you reproduce first on the same surface yourself; hand it to the user only under the narrow exception in Bug fix step 1.
- After opening a PR → use the **babysit** skill when available. If unavailable, inspect checks, review comments, and PR state directly.
- Broken skill mid-task → fix it in its own PR. Don't block. Don't silently work around it.
- Long, autonomous, or multi-phase work, or any task the user steps away from to review later ("going to bed", "trust it when i'm back", "a Codex automation or heartbeat until X") → keep a decision trail via the **show-me-your-work** skill. Commit it when the stakes need an auditable record; keep it local otherwise.
...
**Use Codex subagents only when the available tool policy allows it.** If a playbook asks for a delegate and `multi_agent_v1.spawn_agent` is available, prefer a `worker`, `reviewer`, `architect`, or `default` agent type that matches the task. If subagents are unavailable or not authorized, do the work directly and preserve the review separation yourself.

**Defaults for every `multi_agent_v1.spawn_agent` call.** Keep prompts bounded, pass file pointers instead of inlining large files, assign disjoint write scopes, and omit model overrides unless a specific diverse-model review needs them. Use `gpt-5.3-codex-spark` for cheap coding delegates only when a model override is justified. Use `gpt-5.5` with high or xhigh reasoning for judgment delegates only when the task earns it.
...
Every playbook ends with a reply written this way. If a PR was opened, include the PR link as `https://github.com/<owner>/<repo>/pull/<number>`. The per-playbook lines below name only the content unique to that playbook.
...
Match the task to a playbook below, open its file, and copy its steps into your Codex plan verbatim before reasoning about the task.
...
- **Autonomous run.** A long task to drive to completion without stopping ("run until done", "a Codex automation or heartbeat until X"). Full steps: `playbooks/autonomous-run.md`.
