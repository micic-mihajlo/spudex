...
Produce a phased implementation plan grounded in the **Principles** section of the `spudex` skill. The plan is the deliverable. Do not implement.

Open a Codex `update_plan` with one item per step below.
...
Read the **Principles** section of the `spudex` skill end to end, and the leaf `principle-*` skills it indexes. The principles govern every plan decision; cross-link them.
...
- Prefer `agent_type: "spudex-delegate"`. `default` is the fallback. Never use the built-in `plan` agent_type; it ignores this skill.
- Pass `model:` explicitly. `gpt-5.3-codex-spark` for code reads, `gpt-5.5` for judgment.
...
- For browser / Electron / Web UIs: use the `control-ui` skill from the matching Codex skills or plugins
- For CLIs and TUIs: use the `control-cli` skill from the matching Codex skills or plugins
...
In the overview, name which Spudex non-negotiables the implementer must apply, by name:
...
- `deslop` over each diff before commit when available. Use explicit self-review when it is not. Use the **unslop** skill over any prose surface.
- the **show-me-your-work** skill to keep a decision trail when the plan is large enough to need an auditable record.
- the **babysit** skill when available after opening the PR.
