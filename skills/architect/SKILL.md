...
Open a Codex `update_plan` with one entry per phase before starting work. Autonomous mode without checkpoints needs the list to show phase position and keep phases from silently disappearing.
...
Use diverse runners for Phase B when model overrides are justified. Prefer the inherited model by default; when diversity is useful, choose available Codex models such as `gpt-5.5`, `gpt-5.4`, `gpt-5.3-codex`, and `gpt-5.3-codex-spark`.
