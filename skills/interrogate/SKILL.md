...
Launch all four in a single message using the subagent tool, each with a different model. All four get the same prompt built from the template in `references/reviewer-prompt.md`.

| Subagent | Model |
|----------|-------|
| Reviewer A | `gpt-5.5` |
| Reviewer B | `gpt-5.3-codex` |
| Reviewer C | `gpt-5.5` |
| Reviewer D | `gpt-5.3-codex-spark` |

For each reviewer:
- `agent_type`: `default`
- `model`: the model from the table
- Prompt constraint: read-only; do not edit files.
