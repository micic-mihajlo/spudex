...
If the target is vague (e.g., "why do we do it this way?" with no clear referent), make your best guess from the conversation context. Use currently open files, recent edits and the current workspace context, and what was just discussed. State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the available MCPs from the Codex environment. Use the available-tools map when it is present. Otherwise inspect the `mcps/` directory that Codex exposes for enabled MCP servers.
...
- Prompt constraint: investigative only; do not edit files. Use MCP-capable agent settings when the subagent tool exposes them.
...
- `agent_type`: `default`
- `model`: `gpt-5.5`
- Prompt constraint: synthesis only; do not edit files. Use MCP-capable agent settings when the subagent tool exposes them.
